...
gulp.task('wiredep', function () {
  ...
  return gulp.src('app/index.html')
    ...
    // exclude material design icons scss since we're using mdi sass
    .pipe(wiredep.stream({exclude: ['bower_components/mdi/css']}))
    .pipe(gulp.dest('app/'));
});
...
gulp.task('additional_js', function () {
  ...
  return gulp.src('app/index.html')
    .pipe($.inject(gulp.src('bower_components/moment/locale/nl.js',{read: false, cwd: 'app/'}), {name: 'additional'}))
    .pipe(gulp.dest('app/'));
});
...
gulp.task('bower-fonts', function () {
  ...
  var fontFiles = mainBowerFiles({filter: /\.(eot|svg|ttf|woff|woff2)$/i})
